...
constant int MAX_STEPS = 72;
constant float MAX_DIST = 18.0;
constant float SURF_DIST = 0.0015;
...
float scene(float3 p, float time) {
    ...
    float r1 = 0.64 + 0.08 * sin(t * 1.21 + 0.3);
    float r2 = 0.59 + 0.09 * sin(t * 0.93 + 2.0);
    float r3 = 0.62 + 0.07 * sin(t * 1.34 + 1.4);
    float r4 = 0.58 + 0.10 * sin(t * 0.82 + 3.1);
    float r5 = 0.55 + 0.08 * sin(t * 1.47 + 2.6);
    ...
    d = smin(d, sphere(p - float3(0.06, -0.04, 0.02), 0.68 + 0.05 * sin(t * 0.74)), 0.78);
    ...
    d += (rippleA + rippleB + rippleC) * 0.55;
...
float4 renderLivingNodeSample(float2 position, float2 size, float time) {
    ...
    float spec1 = pow(max(dot(n, half1), 0.0), 44.0);
    float spec2 = pow(max(dot(n, half2), 0.0), 24.0);
    ...
    float colorT = clamp(swirl * 0.32 + radial * 0.32 + heightMix * 0.08 + marbling * 0.28, 0.0, 1.0);
    ...
    color += float3(1.0, 0.95, 0.92) * spec1 * 0.20;
    color += float3(1.0, 0.72, 0.82) * spec2 * 0.08;
    color += mix(float3(0.38, 0.08, 0.92), float3(1.0, 0.28, 0.10), radial) * fresnel * 0.34;

    float surfaceDist = scene(p, time);
    float edgeGlow = exp(-14.0 * abs(surfaceDist));
    float aura = exp(-3.2 * max(scene(ro + rd * max(dist - 0.05, 0.0), time), 0.0));
    color += mix(float3(0.22, 0.04, 0.62), float3(1.0, 0.18, 0.34), radial) * edgeGlow * 0.14;

    float sweep = smoothstep(0.80, 1.0, 0.5 + 0.5 * cos(angle - time * 1.35 + 0.6 * sin(time * 0.7)));
    color += float3(1.0, 0.40, 0.22) * sweep * (1.0 - radial) * 0.24;
    color += float3(0.88, 0.18, 1.0) * (1.0 - sweep) * (1.0 - radial) * 0.05;

    color += mix(float3(0.10, 0.02, 0.30), float3(0.35, 0.04, 0.10), radial) * aura * 0.16;

    float alpha = clamp(0.84 + fresnel * 0.14 + edgeGlow * 0.08, 0.0, 1.0);
    return float4(color * alpha, alpha);
}

[[ stitchable ]] half4 livingNodeOrb(float2 position, half4 currentColor, float2 size, float time) {
    ...
    for (int i = 0; i < 9; i++) {
        accumulated += renderLivingNodeSample(position + sampleOffsets[i], size, time);
    }

    float4 color = accumulated / 9.0;
